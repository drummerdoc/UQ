...
C*****END PRECISION > SINGLE
     +   BUFFER, RATIO, RATIO1, RATIO2, TOLER0,
     +   TOLER1, TOLER2, U, UPPER, X
      INTEGER
     +   COMPS, LEVELD, LEVELM, POINTS, VARY1, VARY2, WEIGHT, PADD, PMAX,
     +   TEXT
      LOGICAL
     +   ACTIVE, ERROR, MARK, NEWX, SUCCES
...
C      REAL
C*****END PRECISION > SINGLE
     +   DIFFER, LEFT, LENGTH, LOWER, MAXMAG, MEAN, RANGE,
     +   RIGHT, TEMP, TEMP1, TEMP2
      EXTERNAL
     +   TWCOPY
      INTEGER, save ::
     +   ACT, COUNT, FORMER, ITEMP, J, K, LEAST,
     +   MORE, MOST, NEW, OLD, ROUTE, SIGNIF, TOTAL
      INTRINSIC
     +   ABS, MAX, MIN
      LOGICAL, save ::
     +   MESS
...
!$omp threadprivate(DIFFER,LEFT,LENGTH,LOWER,MAXMAG,MEAN,RANGE)
!$omp threadprivate(RIGHT,TEMP,TEMP1,TEMP2)
!$omp threadprivate(ACT,COUNT,FORMER,ITEMP,J,K,LEAST)
!$omp threadprivate(MORE,MOST,NEW,OLD,ROUTE,SIGNIF,TOTAL)
!$omp threadprivate(MESS)
