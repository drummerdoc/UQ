...
There reactions can be extremely sensitive to
temperature and are continuously modulated by thermal and species diffusion.
In most
...
The multicomponent Navier-Stokes equations provide a accurate framework for approximating
...
differential mass and energy transport processes.
Thermodynamics is based on ideal gas approximation with higher order fits
...
simulation so that additional simplifications are introduced.
...
Although there are uncertainties associated with all of these elements of the model,
here we will focus on kinetics and transport processes.
...
specific fuels\slash conditions remain
...
\MarginPar{Not sure about this paragraph -- JBB}
...
The difficulty here is that this class of experiments is only able to
address a low dimensional manifold of conditions in chemical phase space.
The interplay of diffusion and reaction in time dependent laminar and
turbulent flames explores a much richer set of conditions in chemical
phase space.  Flames that are of practical interest need to evaluate
chemical rates in regions that are represented in the standard target
experiments used to develop the chemical mechanisms.
A richer set of targets experiments are needed to probe chemical
...
hierachy of experiments that will enable us to enrich the set
...
focus on different aspects of the combustion model.
For example, some
...
experiemental conditions will be well categorized. However,
...
Rather than trying to direclty match experimental data in this case, our goal
will be to match features of the attractor and assess how sensitive
those features are to uncertaintites in parameters.
