...
Difficulties arise as the prior and posterior become mutually singular, i.e. when high-probability 
...
The number of particles required can grow exponentially with the number
...
\MarginPar{JBB:  Unable to parse this sentence}
...
A number of approaches have been used to model grain boundaries.  Here we focus on the stochastic approach 
of Rollett \cite{Rollett04}, which is compatible with the continuum representation of point defect kinetics. 
...
%\end{equation}
Each possible orientation is enumerated {\it{a priori}} into an orientation index. Each lattice site
...
\begin{equation}
  ...
  P( S_i, S_j, \Delta E, T) = \left\{ \begin{aligned}
      ...
      \right. 
\end{equation}
In the above, the energy between orientation indicies $J$ and mobility $M$ are experimentally derived parameters for each
\MarginPar{JBB: no uncertainty in J and M?}
...
That's OK for MCMC sampling, but not (I don't think) for implicit sampling.}
...
of convection-diffusion and Butler-Volmer equations. 
This model uses approximately 25 parameters.
...
Different experiments yield quite different estimates.
For example, \cite{Less:2012} cites estimates of the same parameter that differ by an order of 
magnitude.
Thus, fixed parameter simulations are problematic: first, they do not propagate uncertainty in 
these parameters through the model; second, they miss the chance to use information \emph{across scales} 
to optimally understand
