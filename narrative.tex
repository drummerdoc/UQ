...
=======
MC sampling techniques approximate the posterior distributions that are the basis of Bayesian UQ by a set of samples. However, computing these sample can be very hard. A key issue is that many current MC techniques are not robustly effective for complex, high-dimensional problems. There is a weak analogy to the problem of solving large systems of linear or nonlinear equations.
>>>>>>> d383171136bb229b308757b865a633205d518fc5
...
are only known statistically.  We only
...
%%%%%%%%%%% batteries
\subsubsection*{Lithium-ion batteries}
...
=======
Lithium ion batteries present a truly multiscale system, with important macroscopic behavior
>>>>>>> d383171136bb229b308757b865a633205d518fc5
...
\item
particle-electrolyte interfacial kinetics (Butler-Volmer): $I = \exp(\alpha_a \eta) - \exp(\alpha_c \eta)$.  The overpotential $\eta = \eta(I)$ is a function of current, $I$, of Li across the interface, and $\alpha_a$, $\alpha_c$
are anode and cathode charge transfer coefficients.  This equation can be derived under assumptions from more general chemical kinetic equations.
\item
...
convective velocity (a function of chemical potential), and $R_{c_e}$ is the reaction rate (solution of, e.g., Butler
Volmer). 
\item
electrolyte electric potential (reaction-diffusion) $\frac{\partial \phi}{dt} = \nabla \cdot (D_{\phi} \nabla \phi)  + R_{\phi}$, where $D_{\phi}$ is charge diffusivity in electrolyte, $R_{\phi}$ is a volume averaged current
density entering electrolyte.
\item
electrode stress field $\sigma$ (Hooke's law for chemically active solid) $\sigma_{ij} = C_{ijkl}^{Li} [\epsilon_{kl} - \beta_{kl}(c_e - c_{e_0})] $, where $C$, $\epsilon$, $\beta$ are stress/strain tensors and Vegard coefficient.
\item
electrode equilibrium stress $\sigma$ (constitutive equations) $\nabla \cdot \sigma = 0$ 
\item
...
\item
cell/pack temperature $T$ (thermal transport (diffusion)) $\frac{\partial T}{dt} = \nabla \cdot (D_T \nabla \Phi) + R_T$, where $R_T$ is heat flux from electrode scale.
for 
\end{itemize}
Uncertainty quantification in this context represents an
extreme scale challenge:
each scale alone is a challenging 3D simulation in a heterogeneous domain;
...
Although
<<<<<<< HEAD
(\ref{eq:quench}) provides a quantitative relationship
