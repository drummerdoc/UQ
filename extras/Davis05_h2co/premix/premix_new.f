...
C
     ...
     1           ISENSI='SENSITIVITY', IHSENS='HSENSITIITY')
      ...
      ! RG: Scratch variable to convert species names
      INTEGER II, KSYMSCR(KK*16)
C
C///  INITIALIZATION.
C
      KERR = .FALSE.
...
C*****END precision > single
      ...
      NM = KK + 2
C
C      CALL CKSYMS (CCKWRK, LOUT, KSYM, IERR)

      II=16
      CALL CKSYMS (KSYMSCR, II)
      DO II = 1, KK
          DO J= 1, 16
              KSYM(II)(J:J) = ACHAR(KSYMSCR(16*(II-1)+J))
          ENDDO
          write(*,*) 'Read species name for ', 
     1         II, 'as : ', trim(KSYM(II))
      ENDDO
...
C
CDBGC       Form the chemical rate terms
CDBG        S(NT,J) = 1300
CDBG        S(NY+1-1,J) = 1.08d-2
CDBG        S(NY+2-1,J) = 5.32d-8
CDBG        S(NY+3-1,J) = 3.37d-8
CDBG        S(NY+4-1,J) = 0.23
CDBG        S(NY+5-1,J) = 8.98d-6
CDBG        S(NY+6-1,J) = 0.0d0
CDBG        S(NY+7-1,J) = 1.74d-7
CDBG        S(NY+8-1,J) = 1.79d-7
CDBG        S(NY+9-1,J) = 0.76
        CALL CKWYP (P, S(NT,J), S(NY,J), ICKWRK, RCKWRK, WDOT)
CDBG        WRITE(*,*) 'Reaction rates for ', KK, ' species, gridpoint ', J
CDBG        IF( J .eq. 2 ) then
CDBG            DO I=1,KK
CDBG                write(*,*) I, S(NT,J), S(NY+I-1,J), WDOT(I)
CDBG            ENDDO
CDBG        ENDIF
CDBG        STOP
